-- Prevent the Add Relationship flow from silently creating a second person
-- when an exact-name match already exists in the same family.
...
begin
  ...
  if existing_relative_id is not null then
    ...
    where p.family_id = target_family_id
      and lower(btrim(p.display_name)) = lower(btrim(new_relative_name))
    order by p.created_at asc
    limit 1;

    if matching_person_id is not null then
      raise exception 'A person with this name already exists in this family. Connect the existing person instead.';
    end if;
...
revoke execute on function public.create_relationship(uuid, uuid, text, uuid, text, text, text, text) from public, anon;

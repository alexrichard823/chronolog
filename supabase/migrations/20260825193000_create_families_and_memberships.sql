...
-- A family can never have more than one Owner membership.
-- Family creation must insert the family and its Owner membership in the same transaction,
-- which guarantees the PRD requirement of exactly one Owner at the application workflow level.
create unique index family_memberships_one_owner_per_family
  on public.family_memberships (family_id)
  where role = 'owner';

-- Common lookup paths: "which families does this user belong to?" and

...
| IMP-006 | Email / Deliverability | Medium | Planned | Phase 9 email setup | Move authentication email to a dedicated sending subdomain | Before scaling email volume or adding marketing email, use a dedicated auth sending domain such as **auth.getchronolog.com** with a sender such as **no-reply@auth.getchronolog.com**. Keep marketing email on a separate sending domain/subdomain so a reputation problem in one stream does not affect the other. |
...
- **2026-08-28** — Added dedicated authentication email subdomain as a future deliverability/reputation enhancement after configuring Resend on getchronolog.com.

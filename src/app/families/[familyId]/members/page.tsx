...
export default async function FamilyMembersPage({ params, searchParams }: Props) {
  ...
  if (!currentRole || !["owner", "admin"].includes(currentRole)) redirect(`/families/${familyId}?error=no-member-manage-access`);
  ...
  const errorMessages: Record<string, string> = {
    ...
    "no-manage-access": "Only the Owner or an Admin can manage family members.",
  };
  ...
  return (
    <main className="mx-auto w-full max-w-4xl p-8">
      ...
      <div className="mt-6">
        ...
        <p className="mt-2 text-gray-600">Invite relatives, assign roles, and remove access.</p>
      </div>
      ...
      <section className="mt-8 rounded-xl border p-6">
        ...
        <form action={inviteFamilyMember} className="mt-5 grid gap-4 sm:grid-cols-[1fr_160px_auto] sm:items-end">
          ...
          <div>
            ...
            <input id="email" name="email" type="email" required maxLength={320} className="mt-2 w-full rounded border px-3 py-2" placeholder="relative@example.com" />
          </div>
          <div>
            <label htmlFor="role" className="block text-sm font-medium">Role</label>
            <select id="role" name="role" defaultValue="editor" className="mt-2 w-full rounded border px-3 py-2">
        ...
        <p className="mt-4 text-xs text-gray-500">Admins can manage members and family settings. Editors can change family content. Viewers are read-only.</p>
      </section>
            ...
            {invitations.map((invitation) => (
              <div key={invitation.invitation_id} className="flex flex-col gap-3 rounded-lg border p-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500">{roleLabel(invitation.role)} · {roleLabel(invitation.status)} · Expires {new Date(invitation.expires_at).toLocaleDateString()}</p>
                </div>
                {invitation.status === "pending" && (
                  <ConfirmDeleteButton action={revokeInvitation} fields={{ familyId, invitationId: invitation.invitation_id }} confirmMessage={`Revoke the invitation for ${invitation.invited_email}?`} label="Revoke" />
                )}
              </div>
            ))}
          </div>
        ) : <p className="mt-4 text-gray-500">No invitations yet.</p>}
      </section>
      ...
      <section className="mt-8 rounded-xl border p-6">
        ...
        {activity.length ? (
          <ul className="mt-5 space-y-3">
            {activity.map((item, index) => <li key={`${item.created_at}-${index}`} className="text-sm"><p>{activityText(item)}</p><p className="mt-1 text-xs text-gray-500">{new Date(item.created_at).toLocaleString()}</p></li>)}
          </ul>
        ) : <p className="mt-4 text-gray-500">No collaboration activity yet.</p>}
      </section>
    </main>
  );
}

...
type FamilyDashboardPageProps = {
  ...
  searchParams: Promise<{ storyCreated?: string; eventDeleted?: string; storyDeleted?: string; error?: string }>;
};

export default async function FamilyDashboardPage({ params, searchParams }: FamilyDashboardPageProps) {
  const { familyId } = await params;
  const { storyCreated, eventDeleted, storyDeleted, error: errorCode } = await searchParams;

    ...
    <main className="mx-auto w-full max-w-2xl p-8">
      ...
      <form action={updateRelationship} className="mt-8 space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div><label htmlFor="personAId" className="block text-sm font-medium">Person 1 {relationship.relationship_type === "parent_child" ? "(parent)" : ""}</label><select id="personAId" name="personAId" defaultValue={relationship.person_a_id} className="mt-2 w-full rounded border px-3 py-2">{people.map((person) => <option key={person.id} value={person.id}>{person.display_name}</option>)}</select></div>
          <div><label htmlFor="personBId" className="block text-sm font-medium">Person 2 {relationship.relationship_type === "parent_child" ? "(child)" : ""}</label><select id="personBId" name="personBId" defaultValue={relationship.person_b_id} className="mt-2 w-full rounded border px-3 py-2">{people.map((person) => <option key={person.id} value={person.id}>{person.display_name}</option>)}</select></div>
        </div>

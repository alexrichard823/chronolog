...
export default async function MediaDetailPage({ params, searchParams }: Props) {
  ...
  const [familyResult, mediaResult, peopleLinksResult, eventLinksResult, storyLinksResult, membershipResult] = await Promise.all([
    supabase.from("families").select("id, name").eq("id", familyId).maybeSingle(),
    supabase.from("media_items").select("id, title, description, media_type, storage_path, original_filename, mime_type, file_size_bytes, date_captured, created_at").eq("id", mediaId).eq("family_id", familyId).maybeSingle(),
  ...
  return (
    <main className="mx-auto w-full max-w-5xl p-8">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {canEdit && <div className="flex flex-wrap gap-2"><Link href={`/families/${familyId}/media/${mediaId}/edit`} className="rounded border px-3 py-2 text-sm">Edit</Link><Link href={`/families/${familyId}/media/new`} className="rounded border px-3 py-2 text-sm">Add Media</Link><ConfirmDeleteButton action={deleteMedia} fields={{ familyId, mediaId }} confirmMessage={`Delete “${media.title}”? The uploaded file and its ${personIds.length + eventIds.length + storyIds.length} family link(s) will be permanently removed. Linked people, events, and stories will remain.`} label="Delete" /></div>}
      </div>
      ...
      <section className="mt-6 rounded-xl border p-6">
        ...
        <div className="mt-6"><MediaPreview mediaType={media.media_type} signedUrl={signedByPath.get(media.storage_path) ?? null} title={media.title} /></div>
      </section>

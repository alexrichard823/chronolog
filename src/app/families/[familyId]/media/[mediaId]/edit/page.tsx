...
export default async function EditMediaPage({ params, searchParams }: Props) {
  ...
  return <main className="mx-auto w-full max-w-2xl p-8">
    ...
    <form action={updateMedia} className="mt-8 space-y-6">
      ...
      <div className="rounded border bg-surface p-4 text-sm"><span className="font-medium">File:</span> {media.original_filename}</div>
      ...
      <fieldset><legend className="text-sm font-medium">People shown or heard</legend><div className="mt-3 grid gap-2 sm:grid-cols-2">{(peopleResult.data ?? []).map((person) => <label key={person.id} className="flex items-center gap-2 rounded border bg-surface p-3 text-sm"><input type="checkbox" name="personIds" value={person.id} defaultChecked={linkedPeople.has(person.id)} />{person.display_name}</label>)}</div></fieldset>

...
export default async function EditStoryPage({ params, searchParams }: Props) {
  ...
  return <main className="mx-auto w-full max-w-2xl p-8">
    ...
    <form action={updateStory} className="mt-8 space-y-6"><input type="hidden" name="familyId" value={familyId} /><input type="hidden" name="storyId" value={storyId} />
      ...
      <fieldset className="rounded border bg-surface p-4"><legend className="px-1 text-sm font-medium">Story date</legend><select name="datePrecision" defaultValue={story.date_precision} className="mt-2 w-full rounded border px-3 py-2"><option value="unknown">Unknown</option><option value="exact">Exact date</option><option value="approximate">Approximate year</option><option value="range">Date range</option></select><div className="mt-4 grid gap-4 sm:grid-cols-2"><div><label className="block text-sm">Exact date</label><input type="date" name="exactDate" defaultValue={story.date_precision === "exact" ? story.date_start ?? "" : ""} className="mt-2 w-full rounded border px-3 py-2" /></div><div><label className="block text-sm">Approximate year</label><input type="number" min="1" max="9999" name="approximateYear" defaultValue={story.date_precision === "approximate" ? story.date_start?.slice(0, 4) ?? "" : ""} className="mt-2 w-full rounded border px-3 py-2" /></div><div><label className="block text-sm">Range start</label><input type="date" name="rangeStart" defaultValue={story.date_precision === "range" ? story.date_start ?? "" : ""} className="mt-2 w-full rounded border px-3 py-2" /></div><div><label className="block text-sm">Range end</label><input type="date" name="rangeEnd" defaultValue={story.date_precision === "range" ? story.date_end ?? "" : ""} className="mt-2 w-full rounded border px-3 py-2" /></div></div><p className="mt-3 text-xs text-gray-500">Only the fields for the selected precision are used.</p></fieldset>
      ...
      <fieldset><legend className="text-sm font-medium">People in this story</legend><div className="mt-3 grid gap-2 sm:grid-cols-2">{(peopleResult.data ?? []).map((person) => <label key={person.id} className="flex items-center gap-2 rounded border bg-surface p-3 text-sm"><input type="checkbox" name="personIds" value={person.id} defaultChecked={linkedPeople.has(person.id)} />{person.display_name}</label>)}</div></fieldset>
      <fieldset><legend className="text-sm font-medium">Related events</legend><div className="mt-3 space-y-2">{(eventsResult.data ?? []).map((event) => <label key={event.id} className="flex items-center gap-2 rounded border bg-surface p-3 text-sm"><input type="checkbox" name="eventIds" value={event.id} defaultChecked={linkedEvents.has(event.id)} /><span>{event.title}{event.date_display ? ` — ${event.date_display}` : ""}</span></label>)}</div></fieldset>
      <div className="flex gap-3"><button type="submit" className="rounded bg-primary hover:bg-primary-hover px-4 py-2 text-white">Save changes</button><Link href={`/families/${familyId}/stories/${storyId}`} className="rounded border px-4 py-2">Cancel</Link></div>
    </form>
  </main>;
}

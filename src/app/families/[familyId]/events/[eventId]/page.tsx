...
export default async function EventDetailPage({ params, searchParams }: Props) {
  ...
  const [familyResult, eventResult, peopleLinksResult, storyLinksResult] = await Promise.all([
  ...
  const [peopleResult, storiesResult] = await Promise.all([
  ...
  return <main className="mx-auto w-full max-w-4xl p-8">
    ...
    <section className="mt-6 rounded-xl border p-6"><p className="text-sm font-medium uppercase tracking-wide text-gray-500">{event.event_type.replaceAll("_", " ")}</p><h1 className="mt-1 text-3xl font-semibold">{event.title}</h1><p className="mt-3 text-gray-600">{event.date_display || "Date unknown"}{event.place_name ? ` · ${event.place_name}` : ""}</p>{event.description && <p className="mt-5 whitespace-pre-wrap text-gray-700">{event.description}</p>}</section>

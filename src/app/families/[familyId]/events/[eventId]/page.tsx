...
type Props = { params: Promise<{ familyId: string; eventId: string }>; searchParams: Promise<{ created?: string; updated?: string; error?: string }> };
...
export default async function EventDetailPage({ params, searchParams }: Props) {
  ...
  const [familyResult, eventResult, peopleLinksResult, storyLinksResult, mediaLinksResult, membershipResult] = await Promise.all([
  ...
  return <main className="mx-auto w-full max-w-4xl p-8">
    ...
    {updated === "1" && <p className="mt-6 rounded border border-green-200 bg-green-50 p-4 text-sm text-green-800">Event updated successfully.</p>}
    {errorCode === "delete-failed" && <p className="mt-6 rounded border border-red-200 bg-red-50 p-4 text-sm text-red-700">We could not delete this event.</p>}
    <section className="mt-6 rounded-xl border bg-surface p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-sm font-medium uppercase tracking-wide text-gray-500">{event.event_type.replaceAll("_", " ")}</p><h1 className="mt-1 text-3xl font-semibold">{event.title}</h1><p className="mt-3 text-gray-600">{event.date_display || "Date unknown"}{event.place_name ? ` · ${event.place_name}` : ""}</p></div>{canEdit && <div className="flex flex-wrap gap-2"><Link href={`/families/${familyId}/events/${eventId}/edit`} className="rounded border px-4 py-2 text-center">Edit</Link><Link href={`/families/${familyId}/media/new?eventId=${eventId}`} className="rounded border px-4 py-2 text-center">Add Media</Link><ConfirmDeleteButton action={deleteEvent} fields={{ familyId, eventId }} confirmMessage={`Delete “${event.title}”? Its ${personIds.length} person link(s), ${storyIds.length} story link(s), and ${mediaIds.length} media link(s) will be removed. The linked people, stories, and media themselves will remain.`} label="Delete" /></div>}</div>{event.description && <p className="mt-5 whitespace-pre-wrap text-gray-700">{event.description}</p>}</section>
    <section className="mt-6 rounded-xl border bg-surface p-6"><h2 className="text-lg font-semibold">People involved</h2>{(peopleResult.data ?? []).length ? <ul className="mt-4 space-y-2">{(peopleResult.data ?? []).map((person) => <li key={person.id}><Link className="underline" href={`/families/${familyId}/people/${person.id}`}>{person.display_name}</Link></li>)}</ul> : <p className="mt-4 text-gray-500">No people connected yet.</p>}</section>
    <section className="mt-6 rounded-xl border bg-surface p-6"><div className="flex items-center justify-between gap-4"><h2 className="text-lg font-semibold">Stories about this event</h2>{canEdit && <Link href={`/families/${familyId}/stories/new?eventId=${eventId}`} className="rounded bg-primary hover:bg-primary-hover px-3 py-2 text-sm text-white">Add story</Link>}</div>{(storiesResult.data ?? []).length ? <div className="mt-4 space-y-4">{(storiesResult.data ?? []).map((story) => <article key={story.id} className="rounded border bg-surface p-4"><h3 className="font-semibold"><Link className="underline" href={`/families/${familyId}/stories/${story.id}`}>{story.title}</Link></h3><p className="mt-1 text-sm text-gray-500">{story.date_display || "Date unknown"}</p><p className="mt-3 whitespace-pre-wrap text-gray-700">{story.content}</p></article>)}</div> : <p className="mt-4 text-gray-500">No stories are linked to this event yet.</p>}</section>
    <section className="mt-6 rounded-xl border bg-surface p-6"><div className="flex items-center justify-between gap-4"><h2 className="text-lg font-semibold">Media</h2>{canEdit && <Link href={`/families/${familyId}/media/new?eventId=${eventId}`} className="text-sm underline">Attach media</Link>}</div>{media.length ? <div className="mt-5 grid gap-5 sm:grid-cols-2">{media.map((item) => <article key={item.id} className="rounded-lg border bg-surface p-4"><MediaPreview mediaType={item.media_type} signedUrl={signedByPath.get(item.storage_path) ?? null} title={item.title} compact /><h3 className="mt-3 font-semibold"><Link className="underline" href={`/families/${familyId}/media/${item.id}`}>{item.title}</Link></h3>{item.description && <p className="mt-2 line-clamp-3 text-sm text-gray-600">{item.description}</p>}</article>)}</div> : <p className="mt-4 text-gray-500">No media is linked to this event yet.</p>}</section>
  </main>;
}

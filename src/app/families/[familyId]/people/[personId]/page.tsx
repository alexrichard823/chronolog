...
export default async function PersonProfilePage({ params, searchParams }: Props) {
  ...
  return <main className="mx-auto w-full max-w-4xl p-4 sm:p-8">
    ...
    <section className="mt-6 rounded-xl border p-6"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-semibold">Relationships</h2>{canEdit && <ButtonLink href={`/families/${familyId}/people/${personId}/relationships/new`} variant="secondary">Add Relationship</ButtonLink>}</div>{relationshipResult.error ? <p className="mt-4 text-sm text-red-700">We could not load relationships right now.</p> : <div className="mt-5 grid gap-6 md:grid-cols-3">{relationshipGroups.map(([label, items]) => <div key={label}><h3 className="font-medium">{label}</h3>{items.length ? <ul className="mt-3 space-y-3">{items.map(({ relationship, person: relative }) => <li key={relationship.id} className="rounded border p-3"><Link href={`/families/${familyId}/people/${relative.id}`} className="font-medium underline">{relative.display_name}</Link>{readableLabel(relationship.relationship_type === "spouse_partner" ? relationship.partner_status : relationship.parent_child_subtype) && <p className="mt-1 text-xs capitalize text-gray-500">{readableLabel(relationship.relationship_type === "spouse_partner" ? relationship.partner_status : relationship.parent_child_subtype)}</p>}{canEdit && <div className="mt-3 flex flex-wrap items-center gap-2"><Link href={`/families/${familyId}/relationships/${relationship.id}/edit?returnPersonId=${personId}`} className="text-xs underline">Edit</Link><ConfirmDeleteButton action={deleteRelationship} fields={{ familyId, relationshipId: relationship.id, returnPersonId: personId }} confirmMessage={`Delete the relationship between ${person.display_name} and ${relative.display_name}? Neither person will be deleted.`} label="Delete" className="text-xs text-red-700 underline" /></div>}</li>)}</ul> : <p className="mt-3 text-sm text-gray-500">None added</p>}</div>)}</div>}</section>

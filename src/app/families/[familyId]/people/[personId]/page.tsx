...
export default async function PersonProfilePage({ params, searchParams }: Props) {
  ...
  return <main className="mx-auto w-full max-w-4xl p-8">
    ...
    <section className="mt-6 rounded-xl border p-6"><div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-sm font-medium uppercase tracking-wide text-gray-500">Person profile</p><h1 className="mt-1 text-3xl font-semibold">{person.display_name}</h1>{componentName && componentName !== person.display_name && <p className="mt-2 text-gray-600">{componentName}</p>}{person.nickname && <p className="mt-1 text-gray-600">Nickname: {person.nickname}</p>}<p className="mt-3 text-sm capitalize text-gray-600">{person.life_status === "unknown" ? "Life status unknown" : person.life_status}</p></div><div className="flex flex-wrap gap-2"><Link href={`/families/${familyId}/timeline?person=${personId}`} className="rounded border px-4 py-2">Timeline</Link><Link href={`/families/${familyId}/tree?person=${personId}`} className="rounded border px-4 py-2">Tree</Link>{canEdit && <><Link href={`/families/${familyId}/people/${personId}/edit`} className="rounded border px-4 py-2">Edit</Link><Link href={`/families/${familyId}/stories/new?personId=${personId}`} className="rounded border px-4 py-2">Add Story</Link><Link href={`/families/${familyId}/events/new?personId=${personId}`} className="rounded border px-4 py-2">Add Event</Link><Link href={`/families/${familyId}/media/new?personId=${personId}`} className="rounded border px-4 py-2">Add Media</Link><Link href={`/families/${familyId}/people/${personId}/relationships/new`} className="rounded bg-black px-4 py-2 text-white">Add Relationship</Link><ConfirmDeleteButton action={deletePerson} fields={{ familyId, personId }} confirmMessage={`Delete ${person.display_name}? This will remove ${relationships.length} relationship(s), ${eventIds.length} event link(s), ${storyIds.length} story link(s), and ${mediaIds.length} media link(s). The linked events, stories, and media themselves will remain.`} label="Delete" /></>}</div></div></section>
